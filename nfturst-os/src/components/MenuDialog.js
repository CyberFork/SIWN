...
function Dialog(props) {
    ...
    const isOwn = props.store.user.isLogin && props.data.nftCode === props.store.user.userInfo.nftCode;
    return (
        <div ref={el} style={style}>
            <div className="post-left-menu">
                <div onClick={jump}><AlertOutlined />Report</div>
                {isOwn && !IS_TODO  &&<div onClick={() => props.dispatch({type: 'showPostModal', data: {...props.data, isEdit: true}})}><EditOutlined />Edit</div>}
...
Dialog.show = function(e, props) {
    ...
    changePosition(remToPx(1), o.height + 20);
